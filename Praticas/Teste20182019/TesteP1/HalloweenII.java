import java.util.*;
/* Estado: Terminado
|  MooshackDAA- Teste 1- Prob C - Halloween (2018)            |
|     Objetivo:                                               |
...
|           +"(existindo mais X)"
|Nota: para empates de abóboras escolhemos o com maior ind    |
*/
...
public class HalloweenII{
  ...
  public static void bfsAboboraMaxima(Grafo0 g,int s){
    int n=g.num_vertices(),v,w;
    int max_abo=0,ind_max=s,rep=0;
    ...
    while(!lista.isEmpty()){
      ...
      if(aboboras_loja[v]>max_abo){
        max_abo=aboboras_loja[v]; ind_max=v; rep=0;
      }
      else if(aboboras_loja[v]==max_abo){
        ind_max=Math.max(ind_max,v); rep++;
      }
    ...
    if(aboboras_loja[ind_max]==0) System.out.println("IMPOSSIVEL");
    else System.out.println(ind_max+" (existindo mais "+rep+")");
    return;
  }
  public static void main(String[] args) {
    ...
    for(int i=0;i<k;i++){
      tmp=in.nextInt();
      if(aboboras_loja[tmp]!=0)
        System.out.println(tmp);
      else
        bfsAboboraMaxima(g,tmp);
    }
  }
}
